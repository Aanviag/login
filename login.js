...
function initForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (!email || !password || (mode === "signup" && !nameInput.value.trim())) {
      note.textContent = "Fill in every field to continue.";
      note.className = "form-note error";
      return;
    }

    // TODO: replace with a real call to your auth backend, e.g.
    //   const res = await fetch('/api/auth/' + mode, { method: 'POST', body: JSON.stringify({ email, password }) });
    // For now there's no backend, so we just simulate success and move on.
